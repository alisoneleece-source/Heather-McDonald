/* Consistent navigation for the independent Heather McDonald redesign. Desktop navigation is unchanged. */
(() => {
  const css = `@media(max-width:700px){
    header.unified-mobile-header{position:relative!important;z-index:100!important;background:#251925!important;color:#fff!important;border-bottom:1px solid #ffffff25!important;overflow:visible!important}
  ...
  const init=()=>{
    ...
    if(!header || header.querySelector('.unified-mobile-shell')) return;
    header.classList.add('unified-mobile-header');
    const shell=document.createElement('div');shell.className='unified-mobile-shell';
    shell.innerHTML=`<div class="unified-mobile-social" aria-label="Heather's social channels">
      <a href="https://www.instagram.com/heathermcdonald/" target="_blank" rel="noopener noreferrer" aria-label="Instagram"><svg viewBox="0 0 24 24" aria-hidden="true"><rect x="3" y="3" width="18" height="18" rx="5"/><circle cx="12" cy="12" r="4"/><circle cx="17.5" cy="6.5" r=".8" fill="currentColor" stroke="none"/></svg></a>
      <a class="youtube" href="https://www.youtube.com/@JuicyScoop" target="_blank" rel="noopener noreferrer" aria-label="YouTube"><svg viewBox="0 0 24 24" aria-hidden="true"><path d="M21.1 7.2c-.2-.8-.8-1.4-1.6-1.6C18 5.2 12 5.2 12 5.2s-6 0-7.5.4c-.8.2-1.4.8-1.6 1.6C2.5 8.7 2.5 12 2.5 12s0 3.3.4 4.8c.2.8.8 1.4 1.6 1.6 1.5.4 7.5.4 7.5.4s6 0 7.5-.4c.8-.2 1.4-.8 1.6-1.6.4-1.5.4-4.8.4-4.8s0-3.3-.4-4.8ZM10 15.6V8.4l6.2 3.6L10 15.6Z"/></svg></a>
      <a href="https://www.tiktok.com/@heathermcdonald" target="_blank" rel="noopener noreferrer" aria-label="TikTok"><svg viewBox="0 0 24 24" aria-hidden="true"><path d="M14 3v11.8a4.2 4.2 0 1 1-4.1-4.2"/><path d="M14 3c.6 3.3 2.7 5.1 6.1 5.2"/></svg></a>
    </div><div class="unified-mobile-top"><a class="unified-mobile-brand" href="index.html">HEATHER McDONALD<small>the juicy new look</small></a><button class="unified-mobile-toggle" type="button" aria-expanded="false" aria-controls="unified-mobile-menu">Menu ☰</button></div>
    <nav class="unified-mobile-panel" id="unified-mobile-menu" aria-label="Mobile navigation" hidden>
      <a href="index.html">Home</a><details><summary>The Scoop ⌄</summary><div><a href="podcast.html">Podcasts</a><a href="https://www.youtube.com/@JuicyScoop" target="_blank" rel="noopener noreferrer">YouTube ↗</a><a href="https://www.patreon.com/cw/juicyscoop" target="_blank" rel="noopener noreferrer">Patreon ↗</a></div></details>
      <a href="tour.html">Tour dates</a><a href="shop.html">Shop</a><a href="contact.html">Contact</a><a class="unified-mobile-join" href="https://www.patreon.com/cw/juicyscoop" target="_blank" rel="noopener noreferrer">Get the inside scoop ↗</a>
    </nav>`;
    header.appendChild(shell);
    const button=shell.querySelector('.unified-mobile-toggle'), panel=shell.querySelector('.unified-mobile-panel');
    function setOpen(open){panel.hidden=!open;button.setAttribute('aria-expanded',String(open));button.textContent=open?'Close ×':'Menu ☰'}
    button.addEventListener('click',()=>setOpen(panel.hidden));
    panel.querySelectorAll('a').forEach(link=>link.addEventListener('click',()=>setOpen(false)));
    document.addEventListener('keydown',e=>{if(e.key==='Escape'&&!panel.hidden){setOpen(false);button.focus()}});
    document.addEventListener('click',e=>{if(!panel.hidden&&!shell.contains(e.target))setOpen(false)});
    const current=location.pathname.split('/').pop()||'index.html';
    panel.querySelectorAll('a').forEach(a=>{if(a.getAttribute('href')===current)a.setAttribute('aria-current','page')});
  };
